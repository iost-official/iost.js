...
class Tx {
    ...
    getSignedTxHash() {
        const hash = sha3.SHA3(256)
        hash.update(this._bytes(2));
        return base58.encode(hash.digest("binary"))
    }
    ...
    _bytes(n) {
        ...
        if (n > 1) {
            c.pushBytes(Buffer(this.referredTx));
            c.pushString(this.publisher);
            c.pushInt(this.publisher_sigs.length);
            for (let i = 0; i < this.publisher_sigs.length; i++) {
                c.pushBytes(this.publisher_sigs[i]._bytes())
            }
        }
        return c._buf
    }
